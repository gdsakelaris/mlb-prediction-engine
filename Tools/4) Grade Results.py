...
Usage:
    python "Tools/4) Grade Results.py"                  # newest in Predictions/
...
def grade(path):
    ...
    # --all can still count the day in its pooled totals.
    try:
        wb.save(path)
        painted = True
...
def main():
    ...
    ap.add_argument("--all", action="store_true",
                    help="grade (paint) EVERY dated workbook in "
                         "Predictions/ in place, printing one summary "
                         "line per day plus the pooled totals and Bets "
                         "tally")
    args = ap.parse_args()
