...
LEGEND = [
    ("Sort order", "TIER first, and the tier is cut on the SCORE itself, "
     ...
     "important for the model, related columns together — LogLoss "
     "(+LLBase, Edge% = the base-relative logloss edge, Brier "
     "+BrierBase), then discrimination (AUC, Lift, Top10%, Base% "
     "context), then calibration (ECE, Slope, Int), then count accuracy "
     "(MAE +MAEBase/MAEGain, Bias, MeanPred/MeanAct), then dispersion "
     "(Disp +DispCal/DispObs), Acc last. Every column is present for "
     "every row even where a row leaves it empty."),
