...
# nine hole 3.31).
# BENCHED: 1 better (rbi_top10, barely past
...
# Bag members are combined in LOG-ODDS space: a probability-space mean
# systematically pulls toward 0.5 and blunts
...
class FamilyBlendBag(MeanBag):
    """Count-head bag whose FAMILIES get unequal weight:
...
class BaggedCal:
    """Day-block bootstrap bag of calibrators: B
...
class BaggedLineCal:
    ...
    rationale as BaggedCal, different serving contract: the
...
# Threshold-ladder coherence (tier-1 mechanics batch): these
...
# franchise renames: the team-keyed PRIOR-season files miss
...
def load_raw():
    ...
    # audit wave: deeper thresholds for the new heads (rbi2/
    ...
    for d in (pdp, pdb):
        if d is not None:
            ...
            # zone-split whiffs (oz_wh): in-zone swings and
            ...
            if "oz_wh" in d.columns:
                ...
                d["z_sw"] = np.nan
                d["z_wh"] = np.nan
    # v2/v3 scrape schema: elite-velo buckets, pitch-class buckets
    ...
    for d in (pdp, pdb):
        ...
        for c in ("fb95_n", "fb95_sw", "fb95_wh",
                  ...
                  # v7 audit wave: 2K x breaking cell
                  "ts_brk_n", "ts_brk_sw", "ts_brk_wh",
                  # v8 damage-on-contact wave: xwOBA-per-BBE
    ...
    if pdp is not None:                # pitcher-only v5 dispersion sums
        for c in ("fb_v2", "rp_n", "rp_x", "rp_x2", "rp_z", "rp_z2",
                  # v6 sequencing/count-state/movement sums:
                  ...
                  # v7 audit wave: stretch split, perceived
    ...
    # team battery quality: playing-time-weighted catcher
    ...
    # paired IL stints: the layoff CAUSE the days-rest gap
    # features cannot see
...
def _starter_asof(gp):
    ...
    # outing shape: career as-of sum of Outs^2 (with
    ...
    # starter venue splits: career as-of sums in home vs
...
def _team_offense_table(gb):
    # AB/H/TB/HBP joined (#19): the team's season OBP/SLG norm is
...
def _team_starter_outs_table(gp):
    """The manager's leash: the TEAM's as-of season average
...
def _team_sched_table(games, parks):
    ...
    same team+date; game 2 = the higher GamePk of the day)."""
...
def _team_bsr_table(gb):
    ...
    before the date. Positive = the offense has been
...
def _park_table(gb, games):
    ...
    # doubles, triples (the triple head's venue rider — deep
...
BIP_SHRINK = {
    ...
    #-12: the two BBType shares the gb/pullair pair misses — line
...
PD_SHRINK = {
    ...
    # zone-split whiffs + elite-velo buckets:
    ...
    # v6 sequencing / count-state / movement (priors
    ...
    # v7 audit wave: the passivity axis (zone-swing +
    ...
    # v8 damage-on-contact wave: xwOBA per BBE by velo band /
    ...
    # (damage falls as velo rises, lowest on offspeed).
...
# v7 audit wave: base-out conditional velo + per-class
...
PD_PITCHER_D = ("swstr", "csw", "wsw", "chase", "zone", "fbv", "zwsw",
                ...
                "tswh", "f32z", "f32b",
                # v6: 0-2 waste, ahead/behind spin usage (the
...
# Times-through-the-order decay: how much worse a starter's
...
# KNOWN LIMITATION: these priors (and the BIP/PD ones
...
SHRINK = {
    ...
    # productive/unproductive outs (train._PRODOUT -> run/rbi):
    ...
    # hit-type rates (audit wave): the triple head's own history
...
# Benched (hrr2_ece 1.5x band, rest flat), RE-ACCEPTED
# (queue Tier B6, keep-leaning bar) with an hrr2 ROUTE-AROUND:
...
# audit wave: deeper thresholds for the heads that shipped
...
#... and his run environment: shrunk as-of total runs per
...
# Starter venue splits: as-of home/road K / HR / ERA for a
...
# Outing-shape gates: starts needed before the outing-
...
# BaseRuns cluster-luck residual: trailing-30-day team
...
# schedule/travel: tz approximation — degrees of
...
# audit wave (rank 28): fit-free v1 extras already carrying
...
# 5.6e-17). Benched (run_ece +.0063 past band, rbi mixed),
# RE-ACCEPTED (queue Tier B5, keep-leaning bar) with a run
...
def _ump_rg_shrink(runs, games):
    """Shrunk runs-per-game with this HP ump — shared by
...
# IL stints: return-from-injury features off the paired
...
def _attach_context(rows, raw, team_tab, pen_tab, park_tab,
    ...
    # K/walk heads drop them), and the team-runs model.
...
def add_batter_derived(df):
    """Row-wise derived batter features + interactions,
    ...
    # air-density carry (humidity + station pressure)
    ...
    # style-collision products: batted-ball outcomes are
    ...
    # ---- exposure products: every prop is
    ...
    # ---- bullpen exposure: every batter prop is secretly
    ...
    # ---- form-weighted exposure: the xpa_x_* products
    ...
    # ---- ride collisions: high-IVB fastballs live at the
    ...
    # ---- audit wave (Model/AUDIT_BUILD_SPECS_0714.md) ----
    ...
    df["ts_brk_matchup"] = ((df["bd_tsbrkwh_d"] - PD_SHRINK["tsbrkwh"][0])
                            * (df["p_brk_d"] - PD_SHRINK["brk"][0]))
    # ---- v8 damage-on-contact collisions: the damage
    ...
    # ---- battery collisions: the opposing catchers' framing
...
def build_batter_frame(raw):
    ...
    # schedule context: the batter's TEAM's previous-
    ...
    wdn2 = np.exp(-DECAY_LAM * (df["Date"] - DECAY_EPOCH)
                  .dt.days.to_numpy(dtype="float64"))
    if raw.get("pdb") is not None:
        # zwsw/fb95wh/v3 splits: NaN day-sums under a pre-
    ...
    if raw.get("oaa") is not None:
        ...
        df["opp_oaa"] = np.nan
    # the OPPONENT's battery, prior season: framing runs per
    ...
    if raw.get("cat") is not None:
        ...
        for c in ("opp_cat_frame", "opp_cat_csaa", "opp_cat_pop",
                  "opp_cat_popc"):
            df[c] = np.nan
    # IL return context: the batter's own last stint and the
    ...
    # runs that raw sprint speed only proxies
    ...
    # MiLB translated prior (Phase-3 rider): the batter's
    ...
    # runners-ahead advancement: mean prior-season extra-base
    ...
    # finish batch — new heads: the four deep binaries (H1,
...
def batter_feature_cols():
    ...
    cols += ["p_starts_career", "p_starts_season", "p_days_rest",
             ...
             *ARS_DYN_COLS,   # audit wave rank 22 (opposing-starter arsenal)
             ...
             # decayed teammate ctx: benched (0/0/76 within
             # noise), RE-ACCEPTED under the keep-leaning bar
             # (queue Tier A2) — routed with _CTX in train.py
             "ctx_ahead_obp_d", "ctx_behind_slg_d",
             # rbi_opp_obp + exposure (xpa_*) UNBENCHED for the
             ...
             # decayed r/rbi form: re-accepted with the rolling
             # variants (RUNRBI_FORM_COLS note; run routed around)
             "d_r_pa_sh", "d_rbi_pa_sh",
             # own H+R+RBI threshold-share history: re-accepted
             ...
             # benched iteration 4, RE-ACCEPTED (queue Tier C)
             ...
             # hand-split contact quality: benched (tb2 ECE),
             # RE-ACCEPTED (queue Tier B4) with a tb2/xtb
             ...
             *BAT_TRACK_COLS,
             # HR-physics + matchup interactions: pull
             ...
             # batter-vs-pitcher direct history: contact-quality +
             ...
             # realized handed park-HR edge: venue HR/PA for the
             ...
             "park_hand_hr_edge",
             # data batch — new sources, selection decides per head:
             ...
             # closing sweep: structures trees can't build —
             ...
             *MILB_BAT_COLS,
             # v3 scrape schema: whiff splits by pitch class +
             ...
             "p_fbv_sd", "p_rel_sd",
             # finish batch (backlog #15/#18/#23/#24/#27/#31):
             ...
             # quality (bvh_*/pvh_*, benched on tb2 ECE) was
             # re-accepted with a tb2/xtb route-around (cols
             # above); exposure features (xpa_bat/xpa_slot,
             # eve) likewise — rbi/single/bk ECE past band, no
             # AUC/edge movement (see the XPA_PRIOR note); own H+R+RBI
             # threshold-share history (c/s/d_hrr{2,3}_g_sh)
             ...
             # lg_* were re-accepted into the batter models
             ...
             # the hand-curated PROP_EXCLUDE routing was deleted.
             # ---- audit wave (AUDIT_BUILD_SPECS_0714.md) ----
             ...
             *MILB_PIT_COLS,
             ...
             # battery + IL wave: the opponent's playing-time-
             ...
             *CAT_COLS]
    # r{w}_{r,rbi}_pa_sh re-accepted (RUNRBI_FORM_COLS note)
    cols += SHRINK_COLS
    return cols
...
def add_starter_derived(df):
    ...
    # v8: the velocity-graded lineup collision (lu_ars_whiff's
    ...
    # layoff / ramp regime flags: thin-support interactions
    ...
    # earned-run conversion chain: baserunners allowed x
    ...
    # ump x the actual lineup's K-proneness: completes the
    ...
    # battery collisions: framing cashes at the shadow zone —
...
def build_starts_frame(raw, batter_frame=None):
    ...
    # the manager's leash: his TEAM's as-of average
    ...
    # schedule context: his team's travel/day-night
    ...
    if batter_frame is not None:
        lu = (batter_frame.groupby(["GamePk", "Team"])
              .agg(lu_k_sh=("s_k_pct_sh", "mean"),
                   ...
                   # v8: the lineup's whiff-by-velo-band and
    ...
    # the starter's own times-through-order decay: how much his
    ...
    # prior-season OAA of his own team's posted lineup)
    if raw.get("oaa_players") is not None:
        ld = _lineup_oaa_table(gb, raw["oaa_players"]).rename(
    ...
    # his OWN battery, prior season: framing (strikes FOR
    ...
    if raw.get("cat") is not None:
        ...
        for c in ("own_cat_frame", "own_cat_csaa", "own_cat_pop"):
            st[c] = np.nan
    # his own IL return context: pairs with the #17 ramp
    ...
    # MiLB translated prior, pitcher-allowed side (rider) —
...
def starts_feature_cols():
    return ["Season", "month", "Home", "p_starts_career", "p_starts_season",
            ...
            *ARS_DYN_COLS,   # audit wave rank 22 (own arsenal dynamics)
            ...
            "ump_k_pct", "ump_bb_pct",
            # data batch: in-zone whiff (pure stuff), air density
            ...
            # v3 scrape schema: whiff induced by pitch class,
            ...
            "pd_fbv_sd", "pd_rel_sd",
            # MiLB translated prior, allowed side (rider)
            *MILB_PIT_COLS,
            # finish batch (backlog #16/#17/#21/#23/#25-28/#29 +
            ...
            "pd_relsep_d",
            # v8 damage-on-contact wave: his own xwOBA-allowed
            ...
            # battery + IL wave: his own catchers' framing /
...
def lineup_aggregates(bf):
    ...
    — as-of career OBP/SLG (the B-lineup gap's
...
def add_team_game_derived(d):
    ...
    lineup is (game-grain sibling of air_fly)."""
...
def build_game_frame(raw, batter_frame=None):
    ...
    # opposing-starter TTO decay for the run-total environment: same table + shared helper as the batter/starts
    ...
    # posted-lineup tables: player-level
    ...
    # + schedule context and the trailing-30-day
    ...
    for side, team_col in [("away", "AwayTeam"), ("home", "HomeTeam")]:
        ...
        # posted-lineup quality/style aggregates:
        ...
        # schedule context + trailing BaseRuns luck; the DH flags are
        # game-level (identical both sides)
    ...
    # finish batch: winner-facing diffs — sequencing luck due to
    ...
    # air-density weather (humidity/pressure scrape) for the
    # run-total environment; Precip wired (#11 close-out — the
    ...
    # HP-ump run environment for the totals surface
    g = _merge_ump(g, raw)
...
def win_feature_cols():
    ...
    # and pushed the winner's logloss BELOW the
    ...
    cols += ["d_win_pct", "d_rd_pg", "d_pyth", "d_ra_pg", "d_r_pg",
             ...
             # unbenched to the winner (superset dev — selection +
             # 2026 confirm decide at ship; the 10k-row overfit caveat stands).
             "d_ps_xwcon_d",
             # finish batch (#19/#20/#32 winner-facing diffs):
...
def build_team_game_frame(gf):
    ...
    for side, opp, home, score in (("away", "home", 0, "AwayScore"),
        ...
        d = pd.DataFrame({
            ...
            # full-surface pass: the ACTUAL defense this offense
            ...
            # finish batch: B-lineup gap (#19), lineup air
...
def team_game_feature_cols():
    ...
    # MAE cost); UNBENCHED (superset dev — selection + 2026 confirm
...
class _LazyGroups:
    ...
    def __init__(self, df, key):
        ...
        # resized the array and flipped a tie (selftest).
...
class Stores:
    ...
    def __init__(self, raw=None, progress=None):
        ...
        # team battery quality, prior-season like team OAA
        self.cat_prior = (
            r["cat"].drop_duplicates(["Team", "Year"], keep="last")
            .set_index(["Team", "Year"])
            if r.get("cat") is not None else None)
        # paired IL stints per player, activation-sorted
        self.il_by_player = (
        ...
        # finish batch: manager's-leash table (#16), trailing
    ...
    def batter_feats(self, pid, date, season, opp_hand=None, home=None):
        ...
        if h is None or h.empty:
            for pre in ["c", "s"]:
                for k in ["hr_pa", "tb_ab", "h_ab", "k_pct",
                          "bb_pct", "iso", "xbh_ab", "obp"]:
                    out[f"{pre}_{k}"] = np.nan
                # frame parity (debut row): cumsum-minus-self SUMS are 0.0
                # (ratios above stay NaN = 0/0), shrunk rates = priors
                out[f"{pre}_PA"] = 0.0
                out[f"{pre}_TB"] = 0.0
            ...
            for w in ROLL_WINDOWS:
                ...
                # frame parity: the frame's shift(1).rolling over an EMPTY
                # history is NaN (not a 0 sum), so the rolling shrunk rates
                # are NaN too — unlike c_/s_ above, whose cumsum-based 0
                # sums genuinely shrink to the priors
                out.update({f"r{w}_{n}_sh": np.nan for n in SHRINK_ROLL})
    ...
    def catcher_feats(self, team, season, prefix):
        """Prior-season team battery quality: framing runs
    ...
    def il_feats(self, pid, date, pre=""):
        """IL return context — mirror of _il_asof. An unknown
    ...
    def team_sched(self, team, date, venue, day_night):
        """Schedule context off the team's previous game:
